...
解锁条件：
- 默认只解锁两个冠军联盟的角色位；
- 前两个角色位都达到 Rank A 或更高时，自动解锁第三个角色位；
- 前三个角色位都达到 Rank S 或更高时，自动解锁第四个角色位；
- GMS目前还未开放第五、六个角色位。
...
| 困难卡洛斯 | 只有3条命    | 2500                   |
| 困难卡琳  | 最终伤害+20% | 3000                   |
| 困难敌对者  | 最终伤害-20%，被击时能量减少+50% | 10000 |
| 困难林波  | 起始侵蚀值800 | 12500 |
| 困难人马  | 承受伤害+30% | 15000 |
...
$$
\text{伤害结果上限} = \text{武器系数} \times \text{技能系数} \times \text{总属性} \times \text{总攻击力} \times (1+\text{加伤总和}) \times \text{终伤总和} \times \text{爆伤倍率}
$$

上述公式只是上限，实际伤害结果还要乘以一个武器**熟练度**。例如武器熟练度为 80% ，则实际伤害结果为上限的 80% ~ 100% 之间的一个随机数。

然后还需要考虑怪物的**防御率**：$\text{怪物剩余防御率}=\text{怪物默认防御率} \times (1-\text{玩家的无视防御率})$，例如怪物还剩余 20% 的防御率，那么最终伤害结果还要乘以 80% 。4M到老黑的默认防御率是300%，赛轮之后的Boss的默认防御率是380%。

无视防御率不是简单的加算。举个例子，假如我现在有90%的无视防御率打老黑，老黑剩余的防御率是$300\% \times (1-90\%) = 30\%$，如果我再有20%的无视防御率，那么老黑剩余的防御率是$30\% \times (1-20\%) = 24\%$。而我们知道，其实 $24\% = 300\% \times 8\% = 300\% \times (1-92\%)$，所以游戏里会显示为 92% 无视防御率。Boss身上减少防御率的Debuff和玩家自带的无视防御率的效果是一样的。

怪物还有一个**属性抗性**，它的计算方式和防御率与无视防御率是一样的。一般情况下，大部分Boss都有50%的属性抗性，玩家一般只有倾向中有10%的无视属性抗性，个别职业有减少属性抗性的Debuff（魂骑10%、幻灵20%）。

我们逐一分析上述公式中的各个参数：
- 武器系数对于同一种武器是固定的，大部分职业只能使用一种武器，对于多种武器的职业，一般而言双手武器比单手武器系数高。
- 技能系数完全取决于职业。
- $\text{总属性} = \text{主属性} + \text{副属性} \div 4$，而对于每一种属性有：$\text{属性} = \text{基础属性} \times (1 + \text{百分比加成}) + \text{不吃百分比加成的部分}$。一般而言，那种一次性加很多的（例如ARC、SAC、超级属性等）都不吃百分比加成。
- $\text{总攻击力} = \text{基础攻击力} \times (1+\text{百分比加成})$，所有的攻击力都吃百分比加成。
- 加伤总和包含：伤害%、BOSS伤害%、对异常敌人伤害%，这些都是加算的。
- 终伤是乘算的，例如我原先是50%终伤，也就是1.5倍，再来一条50%终伤，那么就是$1.5 \times 1.5 = 2.25$倍，游戏里就会显示 125% 终伤。终伤只与职业有关，不同职业的主动和被动技能有不同的终伤加成。另外就是核心进度（不显示在面板上），但大家满了之后都一样。
- 玩家在0爆伤的情况下，如果发生了暴击，伤害结果会在 120% ~ 150% 波动。每1%的爆伤会将上限和下限都提升1%。

除此之外，还有各种压制也会影响最终伤害结果，这些压制影响都会直接乘算在最终的伤害结果上：
- **等级压制**：当你比怪物高5级时达到满压，最终伤害结果增加20%。
- **星之力压制**：由于后期没有星之力地图，这一项作用不大。
- **ARC压制**：当你比地图的ARC要求高出50%时达到满压，最终伤害结果增加50%。
- **SAC压制**：当你比地图的SAC要求高出50点时达到满压，最终伤害结果增加25%。
