...
为了计算总消耗的分布，我们采用动态规划方法。定义状态 $dp[n][\ell][s]$ 表示经过 $n$ 次升级后，主属性 的等级为 $\ell$，总消耗为 $s$ 的概率。其中：

- $n = 0,1,\dots,20$（升级次数）
- $\ell = 0,1,\dots,10$（主属性 的等级）
- $s$ 为总消耗，是 10 的倍数，范围从 0 到 720。

初始状态：$dp[0][0][0] = 1$。

状态转移：对于每个状态 $dp[n][\ell][s]$，根据 主属性 的当前等级 $\ell$ 查表得到消耗 $c(\ell)$ 和升级 主属性 的概率 $p_主属性(\ell)$。

- 如果 $\ell < 10$：以概率 $p_主属性(\ell)$ 升级 主属性，转移到 $dp[n+1][\ell+1][s + c(\ell)]$。
- 以概率 $1 - p_主属性(\ell)$ 升级 B 或 C，转移到 $dp[n+1][\ell][s + c(\ell)]$。

当 $\ell = 10$ 时，$p_主属性(10) = 0$，只能升级 B 或 C，消耗固定为 $c(10) = 50$。
