...
class LegionSolver {
  ...
  takeFromList(placement: number): number {
  ...
  isPlaceable(position: Point, piece: Piece): Boolean {
    if (!piece) {
      return false;
    }
  ...
  determinePoint(position: Point, piece: Piece, point: Point): number[] {
    let x;
    let y;
  ...
  pause() {
    ...
    if (this.iterations != 0) {
      iterationsStyleVisibility.value = 'visible';
      iterationsValueInnerText.value = `${this.iterations}`;

      timeStyleVisibility.value = 'visible';
      timeValueInnerText.value = `${-this.time}ms`;
    }
  ...
  continue() {
    this.time += new Date().getTime();
    iterationsStyleVisibility.value = 'hidden';
    timeStyleVisibility.value = 'hidden';
    this.pauseResolve!();
...
const iterationsStyleVisibility = ref('')
const iterationsValueInnerText = ref('')
const timeStyleVisibility = ref('')
const timeValueInnerText = ref('')

export { LegionSolver, iterationsStyleVisibility, iterationsValueInnerText, timeStyleVisibility, timeValueInnerText };